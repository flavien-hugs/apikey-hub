...
@pytest.mark.asyncio
async def test_create_api_keys_success_uses_cases(http_client_api, mock_check_assess_allow):
    headers = {"Authorization": "Bearer fake_token"}

    # CASE 1: Create API Key with valid user_id
    case_one_resp = await http_client_api.post("/keys", headers=headers)
    ...
    case_three_resp = await http_client_api.post("/keys")
    ...
    mock_check_assess_allow.assert_called()
    mock_check_assess_allow.assert_called_with(authorization="Bearer fake_token")


@pytest.mark.asyncio
@pytest.mark.parametrize("sort", ["asc", "desc"])
@pytest.mark.parametrize("is_active", [True, False])
async def test_read_all_uses_cases(http_client_api, fake_api_data, fake_data, sort, is_active, mock_check_assess_allow):
    headers = {"Authorization": "Bearer fake_token"}

    create_apikey_resp = await http_client_api.post("/keys", json=fake_api_data, headers=headers)
    ...
    all_resp = await http_client_api.get("/keys", headers=headers)
    assert all_resp.status_code == status.HTTP_200_OK, all_resp.text
    assert all_resp.json().get("total") >= 1

    # CASE 2: Search with sort
    sort_resp = await http_client_api.get("/keys", params={"sort": sort}, headers=headers)
    ...
    user_resp = await http_client_api.get("/keys", params={"user_id": response["user_id"]}, headers=headers)
    assert user_resp.status_code == status.HTTP_200_OK, user_resp.text

    result = user_resp.json()
    assert result.get("total") >= 1
    assert result.get("items")[0]["user_id"] == response["user_id"]

    # CASE 4: Search with invalid 'user_id' param
    invalid_user_resp = await http_client_api.get("/keys", params={"user_id": fake_data.uuid4()}, headers=headers)
    ...
    active_resp = await http_client_api.get("/keys", params={"is_active": is_active}, headers=headers)
    ...
    last_used_at_resp = await http_client_api.get("/keys", params={"last_used_at": last_used_at}, headers=headers)
    ...
    expires_at_resp = await http_client_api.get("/keys", params={"expires_at": expires_at}, headers=headers)
    ...
    created_at_resp = await http_client_api.get("/keys", params={"created_at": created_at}, headers=headers)
...
@pytest.mark.asyncio
async def test_read_one_uses_cases(http_client_api, fake_api_data, mock_check_assess_allow):
    headers = {"Authorization": "Bearer fake_token"}
    ...
    read_resp = await http_client_api.get(f"/keys/{response['_id']}", headers=headers)
    ...
    invalid_id_resp = await http_client_api.get("/keys/invalid-id", headers=headers)
    ...
    non_existent_id_resp = await http_client_api.get("/keys/66f6fd2f9efb2cbc83fbb133", headers=headers)
...
@pytest.mark.asyncio
async def test_regenerate_apikey_uses_cases(http_client_api, fake_api_data, mock_check_assess_allow):
    headers = {"Authorization": "Bearer fake_token"}
    ...
    regenerate_resp = await http_client_api.put(f"/keys/{response['_id']}", headers=headers)
    assert regenerate_resp.status_code == status.HTTP_403_FORBIDDEN, regenerate_resp.text
    assert regenerate_resp.json() == {
        "code_error": "resource/cannot-access-resource",
        "message_error": "You cannot access this resource",
    }

    # CASE 2: Regenerate API Key by invalid ID
    invalid_id_resp = await http_client_api.put("/keys/invalid-id", headers=headers)
    assert invalid_id_resp.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY, invalid_id_resp.text
    assert invalid_id_resp.json()["code_error"] == "app/unprocessable-entity"

    # CASE 3: Regenerate API Key by non-existent ID
    non_existent_id_resp = await http_client_api.put("/keys/66f6fd2f9efb2cbc83fbb133", headers=headers)
...
async def test_activate_or_deactivate_apikey_uses_cases(http_client_api, fake_api_data, action, mock_check_assess_allow):
    headers = {"Authorization": "Bearer fake_token"}
    ...
    update_resp = await http_client_api.put(f"/keys/{response['_id']}/action", params={"action": action}, headers=headers)
    assert update_resp.status_code == status.HTTP_403_FORBIDDEN, update_resp.text
    ...
    invalid_id_resp = await http_client_api.put("/keys/invalid-id/action", params={"action": action}, headers=headers)
    assert invalid_id_resp.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY, invalid_id_resp.text
    assert invalid_id_resp.json()["code_error"] == "app/unprocessable-entity"

    # CASE 3: Update API Key by non-existent ID
    non_existent_id_resp = await http_client_api.put(
        "/keys/66f6fd2f9efb2cbc83fbb133/action", headers=headers, params={"action": action}
    )
    assert non_existent_id_resp.status_code == status.HTTP_400_BAD_REQUEST, non_existent_id_resp.text
    assert non_existent_id_resp.json()["code_error"] == "document/document-not-found"
    assert non_existent_id_resp.json()["message_error"] == "Document not found"
    ...
    invalid_action_resp = await http_client_api.put(
        f"/keys/{response['_id']}/action", headers=headers, params={"action": "invalid-action"}
    )
...
@pytest.mark.asyncio
async def test_remove_apikey_uses_cases(http_client_api, fake_api_data, mock_check_assess_allow):
    headers = {"Authorization": "Bearer fake_token"}
    ...
    delete_resp = await http_client_api.delete(f"/keys/{response['_id']}", headers=headers)
    assert delete_resp.status_code == status.HTTP_204_NO_CONTENT, delete_resp.text

    # CASE 2: Delete API Key by invalid ID
    invalid_id_resp = await http_client_api.delete("/keys/invalid-id", headers=headers)
    assert invalid_id_resp.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY, invalid_id_resp.text
    assert invalid_id_resp.json()["code_error"] == "app/unprocessable-entity"

    # CASE 3: Delete API Key by non-existent ID
    non_existent_id_resp = await http_client_api.delete("/keys/66f6fd2f9efb2cbc83fbb133", headers=headers)
...
async def test_verify_api_key_uses_cases(
    http_client_api, fake_api_data, headers: dict, expected_status: int, expected_verified: bool | None, mock_check_assess_allow
):
    authorization = {"Authorization": "Bearer fake_token"}

    # GIVEN: Créer une clé API
    create_apikey_resp = await http_client_api.post("/keys", json=fake_api_data, headers=authorization)
    assert create_apikey_resp.status_code == status.HTTP_201_CREATED, create_apikey_resp.text
